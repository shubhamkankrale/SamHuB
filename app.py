...
def download_video():
    ...
    # In case of audio, adjust the file extension
    if download_format == 'audioonly':
        file_name = file_name.replace('.webm', '.mp3')  # Assuming the initial file might be .webm

    return send_file(file_name, as_attachment=True)

def progress_hook(d):
    if d['status'] == 'downloading':
        # Check if total_bytes is available to avoid 'NoneType' errors
        total_bytes = d.get('total_bytes', 1)  # Default to 1 to avoid division by zero
        downloaded_bytes = d.get('downloaded_bytes', 0)
        speed = d.get('speed', 0)
        eta = d.get('eta', 'Unknown')
        
        # Calculate progress only if total_bytes is available
